...
-- EXPLORANDO AS INFORMAÇÕES EXISTENTES NO DD (CAT�?LOGO) DO ORACLE
SHOW USER; -- PARA MOSTRAR O USU�?RIO LOGADO
...
C2  CHAR(2) NOT NULL,
C3  VARCHAR2(100),
CONSTRAINT PK_TAB_TUV PRIMARY KEY (C1,C2) -- PRINCIPALMENTE SE A PK FOR COMPOSTA (�NICA FORMA)
);
...
-- VERIFICANDO A CRIA��O DA TABELA
-- UTILIZANDO UM COMANDO DE DESCR��O: DESC (S� EXISTE ORACLE)
-- MOSTRANDO A ESTRUTURA DA TABELA RESUMIDA
DESC TAB_TUV;

-- ACESSANDO O DICION�RIO DE DADOS DIN�MICO (DD)
DESC USER_TABLES;
...
-- VERFICANDO AS REGRAS (CONSTRAINTS) RELACIONADAS � TABELA CRIADA
SELECT *
...
-- MOSTRANDO O �NDICE CORREPONDENTE A PK
SELECT *
FROM USER_INDEXES
WHERE --TABLE_NAME='TAB_TUV'
      TABLE_NAME IN ('TAB_TUV','TAB_TUV2');
...
-- UTILIZANDO A VIEW QUE MOSTRA TODOS OS OBJETOS QUE SEU USU�RIO PORDE VISUALIZAR
DESC ALL_OBJECTS;

SELECT * -- MOSTRANDO TODOS OBJETOS SEM RESTRI��O
FROM ALL_OBJECTS;

SELECT COUNT(*) -- CONTAR OS OBJETOS QUE SEU USU�RIO PODE VER
FROM ALL_OBJECTS;

SELECT COUNT(*) -- CONTAR SOMENTE TABELAS QUE SEU USU�RIO PODE VER
FROM ALL_OBJECTS
WHERE OBJECT_TYPE='TABLE'; -- COMPARA��O DE CONTE�DO NO DD TEM QUE SER EM MAI�SCULAS
...
FROM ALL_OBJECTS
WHERE OBJECT_TYPE='VIEW'
  AND OBJECT_NAME LIKE 'A____T%TAB%';  -- _ (UNDERLINE) SUBSTITUI UMA POSI��O POR QUALQUER CARACTER
  
-- APLICANDO UM FILTRO PARA BUSCAR INFORMA��ES RELACIONADAS NO DD
SELECT OBJECT_NAME
FROM ALL_OBJECTS
WHERE OBJECT_TYPE='VIEW'
...
-- APLICANDO UM FILTRO PARA BUSCAR INFORMA��ES CONSTRAINTS NO DD
SELECT OBJECT_NAME
FROM ALL_OBJECTS
WHERE OBJECT_TYPE='VIEW'
...
INSERT INTO TAB_TUV (C1, C2, C3) VALUES (1,'AB','Testando inclus�o de dados...');
INSERT INTO TAB_TUV (C1, C2, C3) VALUES (2,'Ab','Testando inclus�o de dados...');
INSERT INTO TAB_TUV (C1, C2, C3) VALUES (3,'aB','Testando inclus�o de dados...');
INSERT INTO TAB_TUV (C1, C2, C3) VALUES (4,'AC','Testando inclus�o de dados...');

SELECT *
FROM TAB_TUV;

--COMANDOS PARA DESFAZER OU CONFIRMAR ALTERACOES NO BD
-- UTILIZADOS PARA CONTROLAR AS TRANSA��ES
-- ROLLBACK: DESFAZ TODA A TRANSA��O OU PARTE
-- COMMIT: CONFIRMA TUDO QUE FOI FEITO DE MUDAN�A NO BANCO DE DADOS FISICAMENTE NOS ARQUIVOS
-- SAVEPOINT: POSSIBILITA FRACCIONAR O COMMIT PARA PODER REALIZAR O ROLLBACK SEM PERDER TODOS FEITOS (ROLLBACK TO "SAVEPOINTNAME")
